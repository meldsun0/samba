...
public abstract class RocksDBStorage implements KeyValueStorage {
    ...
    private static final long TIME_TO_ROLL_LOG_FILE_IN_SECONDS = 86_400L; //(1 day = 3600 * 24 seconds)
    ...
    private final WriteOptions tryDeleteOptions = new WriteOptions().setNoSlowdown(true).setIgnoreMissingColumnFamilies(true);
    ...
    public RocksDBStorage(
        ...
        try {
            ...
            ignorableSegments.stream()
                    .filter(
                            ignorableSegment -> existingColumnFamilies.stream().noneMatch(existed -> Arrays.equals(existed, ignorableSegment.getId())))
                    .forEach(trimmedSegments::remove);
            columnDescriptors = trimmedSegments.stream()
