...
class KBucketTest {
    ...
    private final LivenessManager livenessManager = mock(LivenessManager.class);// new LivenessManager(clock, node -> CompletableFuture.completedFuture(null));
    ...
    void addOrUpdate_shouldReplaceExistingNode() {
        final NodeRecord node1 = TestHelper.createNodeRecord();
        final NodeRecord node1Updated = node1.withUpdatedCustomField("Hi", Bytes.fromHexString("0x0000"), KEY_PAIR.secretKey());
    ...
    void addOrUpdate_shouldNotUpdateExistingEntryWhenNewRecordIsOlder() {
        ...
        final NodeRecord nodeUpdated = node.withUpdatedCustomField("Hi", Bytes.fromHexString("0x0000"), KEY_PAIR.secretKey());
    ...
    void addOrUpdate_shouldMoveUpdatedNodeToTheFrontOfTheBucket() {
        final NodeRecord node1 = TestHelper.createNodeRecord();
        final NodeRecord node2 = TestHelper.createNodeRecord();
        final NodeRecord node2Seq2 = node2.withUpdatedCustomField("hello", Bytes.fromHexString("0x1234"), KEY_PAIR.secretKey());
