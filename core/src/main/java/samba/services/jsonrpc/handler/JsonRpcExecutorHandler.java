...
import static samba.services.jsonrpc.handler.AbstractJsonRpcExecutor.handleJsonRpcError;
...
public class JsonRpcExecutorHandler {
  ...
  public static Handler<RoutingContext> handler(
    ...
    return handler(jsonRpcExecutor, jsonRpcConfiguration);
  }
  ...
  public static Handler<RoutingContext> handler(
    ...
    return ctx -> {
      ...
      try {
        createExecutor(jsonRpcExecutor, ctx, jsonRpcConfiguration)
            .ifPresentOrElse(
                executor -> {
                  try {
                    executor.execute();
