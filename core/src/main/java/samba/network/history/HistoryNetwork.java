...
public class HistoryNetwork extends BaseNetwork
                                ...
                                case Content.UTP_CONNECTION_ID -> this.utpService
                                        ...
                                        .thenCompose(
                                                data -> {
                                                    ...
                                                    return SafeFuture.completedFuture(Optional.of(new FindContentResult(data.toHexString(), true)));
                                                });
                                case Content.CONTENT_TYPE ->
                                    // TODO validate content and key before persisting it or responding.-->   historyDB.saveContent(message.getContentKey(), content.getContent());
                                        SafeFuture.completedFuture(Optional.of(new FindContentResult(content.getContent().toHexString(), false)));
                                case Content.ENRS ->
                                    //todo remove us from the list
                                        SafeFuture.completedFuture(Optional.of(new FindContentResult(content.getEnrList())));
    ...
    public PortalWireMessage handleFindContent(NodeRecord srcNode, FindContent findContent) {
        ContentKey contentkey = ContentKey.decode(findContent.getContentKey());

        Optional<byte[]> content =  historyDB.get(contentkey.getContentType(), contentkey.getSszBytes());
        if (content.isEmpty()) {
    ...
    @NotNull
    private static <V>
    Function<Throwable, CompletionStage<Optional<V>>> createDefaultErrorWhenSendingMessage(MessageType message) {
        return error -> {
