...
public class SelectiveAckHeaderExtension extends UtpHeaderExtension {
  ...
  @Override
  public int hashCode() {
    // Use prime numbers for better distribution of hash codes
    int result = 17; // Starting with a prime number
    result = 31 * result + nextExtension; // Incorporate the nextExtension byte
    result = 31 * result + Arrays.hashCode(bitMask); // Incorporate the bitMask array's hash code
    return result;
  }
}
