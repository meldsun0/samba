...
public class PortalHistoryLocalContentTest {
    ...
    public void shouldReturnCorrectResult() {
        ...
        final JsonRpcResponse expected = new JsonRpcSuccessResponse(request.getRequest().getId(), content);
    ...
    public void shouldReturnContentNotFoundResultIfContentIsNotFound() {
        final JsonRpcRequestContext request = createRequest(DefaultContent.key1.toHexString());
        when(historyJsonRpcRequests.getLocalContent(any(ContentKey.class))).thenReturn(Optional.empty());

        final JsonRpcResponse expected = new JsonRpcErrorResponse(request.getRequest().getId(), RpcErrorType.CONTENT_NOT_FOUND_ERROR);
        final JsonRpcResponse actual = method.response(request);
        assertNotNull(actual);
        assertThat(actual).usingRecursiveComparison().isEqualTo(expected);
        verify(this.historyJsonRpcRequests, times(1)).getLocalContent(any(ContentKey.class));
