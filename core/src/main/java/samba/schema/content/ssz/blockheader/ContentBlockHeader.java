...
public class ContentBlockHeader {
    ...
    public ContentBlockHeader(BlockHeader blockHeader, BlockHeaderProofUnion blockHeaderProof) {
        this.blockHeaderWithProofContainer = new BlockHeaderWithProofContainer(blockHeader, blockHeaderProof);
    ...
    public List<Bytes32> getBlockProofHistoricalHashesAccumulator() {
        if (proofType == ContentProofType.BLOCK_PROOF_HISTORICAL_HASHES_ACCUMULATOR) {
            return blockHeaderWithProofContainer.getBlockHeaderProof().getBlockProofHistoricalHashesAccumulator();
        }
        throw new UnsupportedOperationException("Block proof type is not BLOCK_PROOF_HISTORICAL_HASHES_ACCUMULATOR");
    }

    public List<Bytes32> getBeaconBlockProofHistoricalRoots() {
        if (proofType == ContentProofType.BLOCK_PROOF_HISTORICAL_ROOTS) {
            return blockHeaderWithProofContainer.getBlockHeaderProof().getBlockProofHistoricalRootsContainer().getBeaconBlockProofHistoricalRoots();
        }
    ...
    public Bytes32 getBeaconBlockRoot() {
        if (proofType == ContentProofType.BLOCK_PROOF_HISTORICAL_ROOTS) {
            return blockHeaderWithProofContainer.getBlockHeaderProof().getBlockProofHistoricalRootsContainer().getBlockRoot();
        } else if (proofType == ContentProofType.BLOCK_PROOF_HISTORICAL_SUMMARIES) {
            return blockHeaderWithProofContainer.getBlockHeaderProof().getBlockProofHistoricalSummariesContainer().getBlockRoot();
        }
        throw new UnsupportedOperationException("Block proof type is not BLOCK_PROOF_HISTORICAL_ROOTS or BLOCK_PROOF_HISTORICAL_SUMMARIES");
    }

    public List<Bytes32> getExecutionBlockProof() {
        if (proofType == ContentProofType.BLOCK_PROOF_HISTORICAL_ROOTS) {
            return blockHeaderWithProofContainer.getBlockHeaderProof().getBlockProofHistoricalRootsContainer().getExecutionBlockProof();
        } else if (proofType == ContentProofType.BLOCK_PROOF_HISTORICAL_SUMMARIES) {
            return blockHeaderWithProofContainer.getBlockHeaderProof().getBlockProofHistoricalSummariesContainer().getExecutionBlockProof();
        }
        throw new UnsupportedOperationException("Block proof type is not BLOCK_PROOF_HISTORICAL_ROOTS or BLOCK_PROOF_HISTORICAL_SUMMARIES");
    }
    ...
    public long getSlot() {
        if (proofType == ContentProofType.BLOCK_PROOF_HISTORICAL_ROOTS) {
            return blockHeaderWithProofContainer.getBlockHeaderProof().getBlockProofHistoricalRootsContainer().getSlot().longValue();
        } else if (proofType == ContentProofType.BLOCK_PROOF_HISTORICAL_SUMMARIES) {
            return blockHeaderWithProofContainer.getBlockHeaderProof().getBlockProofHistoricalSummariesContainer().getSlot().longValue();
        }
        throw new UnsupportedOperationException("Block proof type is not BLOCK_PROOF_HISTORICAL_ROOTS or BLOCK_PROOF_HISTORICAL_SUMMARIES");
    }
