...
public class GetEnrTest {
    ...
    private final String nodeId = "0xbb19e64f21d50187b61f4c68b2090db0a3283fe54021902822ff6ea0132568be";
    ...
    public void testGetEnrNodeIdDoesNotMatchWithLocalNnodeAndRoutingTableHasNode() throws Exception {
        NodeRecord homeNodeRecord = mock(NodeRecord.class);
        when(discv5Client.getHomeNodeRecord()).thenReturn(homeNodeRecord);
        when(homeNodeRecord.getNodeId()).thenReturn(Bytes.fromHexString("0x0e208c8ab86ad547a60bceeebe64e6f6f2c6fcf37e24943f345731b0c159b5dd"));
        when(homeNodeRecord.asEnr()).thenReturn("enr");

        RoutingTable mockedRoutingTable = mock(RoutingTable.class);
        Field field = ReflectionUtils.findFields(HistoryNetwork.class, f -> f.getName().equals("routingTable"), ReflectionUtils.HierarchyTraversalMode.TOP_DOWN).get(0);
    ...
    public void testGetEnrNodeIdDoesNotMatchAndRoutingTableDoesNotHaveNode() throws Exception {
        ...
        assertFalse(result.isPresent(), "Expected Optional.empty() when node is not found in routing table");
    }
