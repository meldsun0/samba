...
public class NodesContainer extends Container2<NodesContainer, SszByte, SszList<SszByteList>> {
  ...
  public List<String> getEnrs() {
    return getField1().stream()
        .map(sszByteList -> Base64.getUrlEncoder().encodeToString(sszByteList.getBytes().toArray()))
  ...
  public static class NodesSchema
      extends ContainerSchema2<NodesContainer, SszByte, SszList<SszByteList>> {
