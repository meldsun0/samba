...
public class NodeTable {
    ...
    public synchronized Stream<NodeRecord> streamClosestNodes(final Bytes nodeId) {
        return StreamSupport.stream(Spliterators
                .spliteratorUnknownSize(new KBucketsIterator(this, this.homeNode.getNodeId(), nodeId), Spliterator.ORDERED), false);
    }
    ...
    private <T> Stream<T> streamFromBucket(final int distance, final Function<KBucket, Stream<T>> mapper) {
    ...
    private Optional<KBucket> getOrCreateBucket(final int distance) {
        ...
        return Optional.of(buckets.computeIfAbsent(distance, __ -> new KBucket(livenessManager, clock)));
    }
