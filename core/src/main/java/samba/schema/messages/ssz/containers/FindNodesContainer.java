...
public class FindNodesContainer extends Container1<FindNodesContainer, SszByteList> {
  ...
  public FindNodesContainer(Bytes list) {
    super(
        FindNodesSchema.INSTANCE,
        SszByteListSchema.create(PortalWireMessage.MAX_DISTANCES * 2).fromBytes(list));
  }
  ...
  public Set<Integer> getDistances() {
    ...
    for (int i = 0; i < distancesBytes.size(); i += 2) {
      distances.add(
          Bytes.wrap(distancesBytes.slice(i, 2).toArray(ByteOrder.LITTLE_ENDIAN)).toInt());
    }
    return distances;
  }
