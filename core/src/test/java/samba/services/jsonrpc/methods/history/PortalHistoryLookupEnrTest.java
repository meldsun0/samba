...
public class PortalHistoryLookupEnrTest {
    ...
    public void before() {
        ...
        this.method = new PortalHistoryLookupEnr(historyJsonRpc);
    ...
    public void shouldReturnCorrectResult() {
        final JsonRpcRequestContext request = buildJsonRpcRequest(this.nodeRecord.getNodeId().toHexString());

        when(historyJsonRpc.
                lookupEnr(UInt256.fromHexString(this.nodeRecord.getNodeId().toHexString())))
                .thenReturn(Optional.of(this.nodeRecord.asEnr()));
        final JsonRpcResponse expected = new JsonRpcSuccessResponse(request.getRequest().getId(), this.nodeRecord.asEnr());
